...
func _handle_ghost_spawning(delta: float) -> void:
    ...
    if _ghost_spawn_timer <= 0:
        ...
        for ghost in ghosts:
            if !ghost.is_spawned:
                ghost.position = ghost_spawn_marker.position
